$(document).ready(function(){
   ...
   $("select").on("change" , function() {      
      var selection = $(this).find("option:selected").text(), labelFor = $(this).attr("id"), label = $("[for='" + labelFor + "']");
   ...
   var youtube_keys = [
      ...
      "AIzaSyAjtVlR54zAzJ5qfkE-vDTzmQr2XTM_dxw",
      "AIzaSyAr0Bfc10w84YaWoBxIJMEk2svmfJyiPCY",
      "AIzaSyB8VL2ypWk4i1H-ZDwa1M0ezN_U_7tsnlw",
      "AIzaSyBU8UKTBtnkgqg4Jl2p4tXS5Os0zbQpW60",
      "AIzaSyAI2B7Hi8cecBf7afr-mAd0mU8NR7UjIE4",
      "AIzaSyA-zBPFK0r7i4gkcNnmHUoygE7y5biTgl0",
      "AIzaSyCgh7bd3awjyq88yeXaNfyMqShvo-9LgnM",
      "AIzaSyC9NGeSmJPAZv7Mz5WYw3EiNPMk6C4xiGw",
      "AIzaSyDIrXEUWREmh85EuP5IGw9pGeJCX-aOIMM",
      "AIzaSyASYxbZBXZYnXP62GWCojbIeJvAiqOfz-E",
      "AIzaSyA4QpuuP68TLIMRIAwJbDKH1jNf2V9u2Lg",
      "AIzaSyATAO-dQk_lFss17zTQsUiPD5F1Su2HJdk",
      "AIzaSyAt60IF8O86VUFQhLrRAqB3wH9ZTNDNT8g",
      "AIzaSyAEqfwI3DnfUALTRwkQHPEQn2hs0iy4bY4",
      "AIzaSyBueEk81z6f8tBXdFpj5MJ4gSh0ritw2fk",
      "AIzaSyA7L2BO5d73VP3k9-PX8f6IlHBWGA5gjw8",
            ...
            if(data) {
               ...
               data.forEach(function(item, index, arr) {                     
                  ...
                  if(item.type=="csv"||item.type=="xlsx"||item.type=="xls") {
                     data_population[index].image = encodeURIComponent(main_url+'backend/lms/images/excel.svg');
                  } else if(item.type=="pptx"||item.type=="ppt") {
                     data_population[index].image = encodeURIComponent(main_url+'backend/lms/images/powerpoint.svg');
                     ...
                     data_population[index].image = encodeURIComponent(main_url+'backend/lms/images/pdf.svg');
                     ...
                     data_population[index].image = encodeURIComponent(main_url+'backend/lms/images/text.svg');
            ...
            if(data) {
               data.forEach(function(item, index, arr) {                     
                  data_population[index] = {
                     ...
                     image:encodeURIComponent(item.link),
                     type:item.type,
                     source:s3_url+'uploads/lms_cms_resources/'+item.filename,
                     id: item.id,
                  };                     
               });
   ...
   function change_detected() {
      ...
      $.each(jstree.jstree("get_checked",null,true),function(key,value) {         
         if(value.includes('student')) {
            ...
            class_section = value.replace('section_','');
      ...
      lesson_data = {
         ...
         term:term,
         shared:shared,
         grade_section: class_sections.join(','),
      };
   ...
   function populate_search_content(data){
      ...
      data.forEach(function(item, index, arr) {
         ...
         if (item.type!="text") {
            ...
            $(populous).attr("resource_id",item.id);
   ...
   $(document).on("mouseover",".content_result",function() {
      if (active_portal == "my_resources")
         $(this).find(".content_close").show();
   });

   $(document).on("mouseout",".content_result",function() {
      if (active_portal == "my_resources")
         $(this).find(".content_close").hide();
   });
   ...
   $(document).on("click",".content_close",function(){
      ...
      var content_class = $(this).parent().parent().hasClass("content_result");

      if (content_class !== false) {
         //-- ToDo: Delete resource item from the database
         var resourceid = $(this).parent().parent().attr("resource_id");
         ...
         Swal.fire({
            ...
            if (result.isConfirmed) {
               $.ajax({
                  url: url + "my_resources_delete/" + resourceid,
                  method: "POST",
               }).done(function(data) {
