...
class Media extends Admin_Controller
{
      ...
      if (isset($_FILES['files']) && !empty($_FILES['files'])) {
         ...
         if ($responses) {
            ...
            foreach ($responses['images'] as $key => $value) {
               ...
               if ($s3->putObjectFile($destination_path . $value['store_name'], S3_BUCKET, $dest_file, S3::ACL_PUBLIC_READ)) {

                  $s3->putObjectFile($thumb_path . $value['store_name'], S3_BUCKET, $thumb_dest_file, S3::ACL_PUBLIC_READ);
                  ...
                  unlink($destination_path . $value['store_name']);
                  unlink($thumb_path . $value['store_name']);
               }
            }
            if (!empty($img_array)) {
   ...
   function genrateDiv($result, $is_gallery)
   {
      ...
      if ($result->file_type == 'image/png' || $result->file_type == 'image/jpeg' || $result->file_type == 'image/jpeg' || $result->file_type == 'image/jpeg' || $result->file_type == 'image/gif') {
         ...
         $file = $_SESSION['S3_BaseUrl'] . $result->thumb_path . $result->img_name;
         $file_src = $_SESSION['S3_BaseUrl'] . $result->thumb_path . $result->img_name;
         $is_image = 1;
         ...
         $file_src = $_SESSION['S3_BaseUrl'] . $result->dir_path . $result->img_name;
      ...
      $output .= "<img class='' data-fid='" . $result->id . "' data-content_type='" . $result->file_type . "' data-content_name='" . $result->img_name . "' data-is_image='" . $is_image . "' data-vid_url='" . $result->vid_url . "' data-img='" . $_SESSION['S3_BaseUrl'] . $result->dir_path . $result->img_name . "' src='" . $file . "'>";
      $output .= "</div>";
      if ($is_video == 1) {
