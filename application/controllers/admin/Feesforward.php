                ...
                if ($this->form_validation->run() == TRUE) 
                {
                    ...
                    foreach ($counter as $count_key => $count_value) 
                    {
                        ...
                        if ($amount >= 0) {
                            ...
                            // $sender_details = array('student_id' => $student_array['student_session_id'], 'contact_no' => $this->input->post('guardian_phone'), 'email' => $this->input->post('guardian_email'));
                            // $this->mailsmsconf->mailsms('balance_reminder', $sender_details);
                        }
                    }
    ...
    public function findPreviousBalanceFees($session_id, $class_id, $section_id, $current_session) 
    {
        ...
        if (!empty($studentlist)) 
        {
            ...
            foreach ($studentlist as $student_list_key => $student_list_value) 
            {
                ...
                $obj->roll_no = $student_list_value->guardian_email;
