   ...
   function index()
   {
      ...
      if ($this->form_validation->run() == TRUE) {
         ...
         if (isset($_FILES["logo"]) && !empty($_FILES["logo"]['name'])) {
            ...
            // print_r($frontcmslist->logo);
            // die();

            $s3 = new S3(AWS_ACCESS_KEY_ID, AWS_ACCESS_KEY_SECRET, false, S3_URI, AWS_REGION);
            $dest_file = $_SESSION['School_Code'] . "/uploads/school_content/logo/" . $newLogoName;

            if ($s3->putObjectFile($_FILES["logo"]["tmp_name"], S3_BUCKET, $dest_file, S3::ACL_PUBLIC_READ)) {
               $data['logo'] = $logo_dir;
               $s3->deleteObject(S3_BUCKET, $_SESSION['School_Code'] . $frontcmslist->logo);
            }
         }
         if (isset($_FILES["logo"]) && !empty($_FILES["fav_icon"]['name'])) {
            ...
            $s3 = new S3(AWS_ACCESS_KEY_ID, AWS_ACCESS_KEY_SECRET, false, S3_URI, AWS_REGION);
            $dest_file = $_SESSION['School_Code'] . "/uploads/school_content/logo/" . $newFavName;

            if ($s3->putObjectFile($_FILES["fav_icon"]["tmp_name"], S3_BUCKET, $dest_file, S3::ACL_PUBLIC_READ)) {
               $data['fav_icon'] = $fav_dir;
               $s3->deleteObject(S3_BUCKET, $_SESSION['School_Code'] . $frontcmslist->fav_icon);
            }
         }
