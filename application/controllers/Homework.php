...
class Homework extends Admin_Controller
{
   ...
   public function homework_docs2($id, $classid, $sectionid)
   {
      $session_id = $this->setting_model->getCurrentSession();
   ...
   public function edit()
   {
      ...
      if ($this->form_validation->run() == false) {
         ...
         if (isset($_FILES["userfile"]) && !empty($_FILES['userfile']['name'])) {
            ...
            //-- Since S3 no need to create folder
            // if (!is_dir($uploaddir) && !mkdir($uploaddir)) {
            //    die("Error creating folder $uploaddir");
            // }
   ...
   public function assigmnetDownload($doc)
   {
      ...
      $filepath = $_SESSION['S3_BaseUrl'] . "uploads/homework/assignment/" . $this->uri->segment(5);
      $data     = file_get_contents($filepath);
      force_download($name, $data);
   }
}
