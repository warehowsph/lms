...
class Homework extends Student_Controller
{
   ...
   public function submitHomeworkVerify($url_link, $document_file)
   {
      return ($url_link != '' || $this->input->post($document_file) != '');
   }
   ...
   public function upload_docs()
   {
      ...
      $data['url_link'] = $_REQUEST['url_link'];
      ...
      // $this->form_validation->set_rules('url_link', $this->lang->line('url_link'), 'trim|required|xss_clean|callback_handle_upload[' . $is_required . ']');
   ...
   public function download($id, $doc)
   {
      ...
      $name     = $doc;
      $ext      = explode(".", $name);
      $filepath = $_SESSION['S3_BaseUrl'] . "uploads/homework/" . $doc;
   ...
   public function assigmnetDownload($id, $doc)
   {
      ...
      $name     = $this->uri->segment(5);
      // $ext      = explode(".", $name);
      $filepath = $_SESSION['S3_BaseUrl'] . "uploads/homework/assignment/" . $doc;
      $data     = file_get_contents($filepath);
      force_download($name, $data);
   }
   ...
   {
      $image_validate = $this->config->item('file_validate');
      $urllink = $_REQUEST['url_link'];

      if ((isset($_FILES["file"]) && !empty($_FILES['file']['name']) && $_FILES["file"]["size"] > 0) || $urllink != '') {
         ...
         if (isset($_FILES["file"]) && !empty($_FILES['file']['name']) && $_FILES["file"]["size"] > 0) {
            ...
            //-- Validate url link
            // if (filter_var($_REQUEST['url_link'], FILTER_VALIDATE_URL)) {
            //    $this->form_validation->set_message('handle_upload', "URL link not valid.");
            //    return false;
            // }
            return true;
         }
         ...
         if ($is_required == 0) {
            $this->form_validation->set_message('handle_upload', 'Please choose a file to upload or enter a url link of your file.');
            return false;
         } else {
            return true;
         }
