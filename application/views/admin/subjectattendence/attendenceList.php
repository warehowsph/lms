                     ...
                     <div class="box-body">
                        ...
                        if (!empty($resultlist)) {
                           ...
                           <div class="row">
                              <!-- <div class="col-md-3">
                                 <select autofocus="" id="lesson_id" name="lesson_id" class="form-control">
                                    <option value=""><?php echo $this->lang->line('select') . " student lesson logs"; ?></option>
                                    <?php foreach ($lesson_list as $lesson) { ?>
                                       <option value="<?php echo $lesson['id'] ?>" <?php echo set_select('lesson_id', $lesson['id'], set_value('lesson_id')); ?>><?php echo $lesson['lesson_name'] ?></option>
                                    <?php } ?>
                                 </select>
                                 <span class="text-danger"><?php echo form_error('class_id'); ?></span>

                                 <div class="table-responsive ptt10">
                                    <table id="scTable" class="table table-hover table-striped nowrap">
                                       <thead>
                                          <tr>
                                             <th>Lesson Join Time</th>
                                             <th>Zoom/GMeet</th>
                                          </tr>
                                       </thead>
                                       <tbody>
                                    ...
                                    <div class="mailbox-controls">
                                       <span class="button-checkbox">
                                          <?php if ($this->rbac->hasPrivilege('student_attendance', 'can_add')) { ?>
                                             <button type="button" disabled class="btn btn-sm btn-primary" data-color="primary"><?php echo $this->lang->line('mark_as_holiday'); ?></button>
                                                      ...
                                                      foreach ($attendencetypeslist as $key => $type) {
                                                         if ($type['key_value'] != "H") {
                                                            ...
                                                            <?php
                                                               ...
                                                               <div class="radio radio-info radio-inline">
                                                                  <?php if ($value['time_entered'] != null && $value['time_entered'] != '') { ?>
                                                                  ...
                                                                  <?php } else { ?>
                                                                     <input <?php if ($c == 3) echo "checked"; ?> type="radio" id="attendencetype<?php echo $value['student_session_id'] . "-" . $count; ?>" value="<?php echo $type['id'] ?>" name="attendencetype<?php echo $value['student_session_id']; ?>">
                                                                  <?php } ?>
                                                                  <label for="attendencetype<?php echo $value['student_session_id'] . "-" . $count; ?>">
                                                                     <?php echo $this->lang->line($att_type); ?>
                                                                  </label>
...
<script type="text/javascript">
   ...
   $(document).ready(function() {
      ...
      function getLessons(subject_timetable_id, class_id) {
         // var subject_timetable_id = $(this).val();
         var selectedSubject = subjects.find(o => o.id === subject_timetable_id);
         var subject_id = selectedSubject.subject_id;
         var teacher_id = selectedSubject.staff_id;
         // var class_id = $("#class_id").val();
         var url = baseurl + "lms/lesson/get_lesssons_by_level_subject/" + teacher_id + "/" + class_id + "/" + subject_id;
         ...
         $.ajax({
            ...
            success: function(data) {
               ...
               $.each(data, function(i, obj) {
                  var select = "";
                  if (lesson_id_post == obj.id) {
                     var select = "selected=selected";
                  }

                  div_data += "<option value=" + obj.id + " " + select + ">" + obj.lesson_name + "</option>";
               });
      ...
      $(document).on('change', '#subject_timetable_id', function(e) {
         ...
         $.ajax({
            ...
            success: function(data) {
               lesson_list = data;
               $.each(data, function(i, obj) {
                  div_data += "<option value=" + obj.id + ">" + obj.lesson_name + "</option>";
               });
               $('#lesson_id').append(div_data);
            }
         });
      ...
      $(document).on('change', '#lesson_id', function(e) {
         ...
         var url = baseurl + "lms/lesson/get_lesson_logs/" + lesson_id + "/" + class_id_post + "/" + section_id_post;

         manageTable.ajax.url(url).load(null, false);
         // $('#scTable').DataTable.ajax.url(url).load(null, false);
      });
   });
